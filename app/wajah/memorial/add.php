...
<section class="content-header">
	<div class="row">
		<div class="col-md-5">
			<label class="label-header">Tambah Jurnal Umum</label>
		</div>
	</div>
</section>
	...
	<div class="col-md-12">
		<div class="box">
			<div class="box-header">
				 <a href="<?=base_url()?>jurnal"  class="btn btn-primary" type="button" >Kembali ke Daftar Jurnal</a>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	$("#form").submit(function(e){
		...
		while( ( input = document.getElementById( 'debet_'+i ) ) !== null ) {
			totalDebet += parseInt( clearNum(input.value) );
			++i;
		}
		//alert( totalDebet );
		...
		if(totalDebet!=totalKredit){
			...
			return false;
				...
				$.ajax({
					type:'POST',
					url:'<?php echo base_url('/memorial/simpan')?>',
					dataType: "json",
					data:$(this).serialize(),
	...
	function tmp(id){
		...
		html_out += "<input type='text' class='form-control kanan' name='txt_debet[]'   onkeyup='inputnilai_d("+id+")'    id='debet_"+id+"'  value='0' placeholder='Debet'/></td>";
		html_out += "<td><input type='text' class='form-control kanan' id='kredit_"+id+"'   onkeyup='inputnilai_k("+id+")'    name='txt_kredit[]' placeholder='Kredit' value='0'/></td>";
	...
	function inputnilai_d(x){
		$('#debet_'+x).val(formatCurrency($('#debet_'+x).val()));
	}
	function inputnilai_k(x){
