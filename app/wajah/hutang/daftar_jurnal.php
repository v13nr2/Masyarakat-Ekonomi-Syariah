						...
						<div class="row">
							<div class="col-lg-12">
								<div class="col-lg-4 col-xs-10">
									...
									<button type="submit" name="btnreset" value="reset" class="btn btn-danger btn-sm"><i class="fa fa-refresh"></i> Reset</button> <a href="<?=base_url()?>hutang" class="btn btn-sm btn-header btn-success"><img src="<?=base_url();?>assets/resources/add.png" /> Buat Jurnal Baru</a>
								</div>
							</div>
						</div>
					</div>
								...
								<thead>
									...
									<tbody>
										<?php foreach ($jurnal as $b)
										{
											...
											echo '<td><a href="'.base_url('jurnal/detail/'.$b->kode_unik).'">'.$b->kreditur.'</a></td>';
											echo '<td>'.$b->no_bukti.'</td>'; echo '<td>'.$b->memo.'</td>';
											echo '<td class="text-right">'.$b->tgl_dibuat.'</td>';
											echo '<td class="text-right">'.number_format($b->kredit).'</td>';
