...
class Model_services extends CI_Model {
	function get_api_id() {
		...
		$this->db->select('api_web_service');
		$this->db->where('id', $ORG);
		$this->db->from('tbl_organisasi');
		$data = $this->db->get()->row_array();
		
		return $data["api_web_service"];
	}
	
	function list_aktivitas4() {
        ...
        LEFT JOIN trs_jurnal_detail AS Z ON Z.id_akun = A.id_akun WHERE A.kode_akun LIKE '4%' 
	...
	function list_asetnetto() {
        ...
        LEFT JOIN trs_jurnal_detail AS Z ON Z.id_akun = A.id_akun WHERE   A.kode_akun =(SELECT coa_aset_netto FROM tbl_organisasi WHERE id = 1)
        ORDER BY A.kode_akun";
		$query = $this->db->query($SQL);
		return $query->result();
	...
	function list_aktivitas5() {
        ...
        LEFT JOIN trs_jurnal_detail AS Z ON Z.id_akun = A.id_akun WHERE   A.kode_akun LIKE '5%' 
        ORDER BY A.kode_akun";
		$query = $this->db->query($SQL);
		return $query->result();
