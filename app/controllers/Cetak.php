...
class Cetak extends CI_Controller {
	...
	function index()
	{
		...
		$data['jurnal'] 		= $this->model_cetak->list_jurnal_header();
    //	$data['tanggal_akhir'], $data['no_jurnal'], $data['dikonfirmasi'])->result();
		$this->template->view_baru('cetak/daftar_jurnal', $data);
	}
		...
		function generate_to_pdf(){
    		...
    		$header 		= $this->model_cetak->get_data_header($kode_unik)->row_array();
    		$data['header'] = $header;
    		$data['detail'] = $this->model_cetak->get_data_detail($header['id_jurnal'])->result();
    		$data['judul'] 	= "Detail Jurnal " . $header['no_jurnal'];
    		$data['back'] 	= $this->uri->segment(1);
			$this->pdf->load_view('cetak/detail_jurnal_cetak_pdf', $data);
			$this->pdf->render();
			$this->pdf->stream("bukti-transaksi.pdf");
		}
	...
	public function detail()
	{
		...
		$this->template->view_baru('cetak/detail_jurnal', $data);
	}
