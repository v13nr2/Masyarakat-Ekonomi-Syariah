...
class Aset extends CI_Controller {
	...
	public function insert(){
		...
		$data = array(
				  ...
				  'aktiva_id' => $id_aktiva,
				  'nilai' => $nilairata,
				  'mano_post' => $akhirbulan
			);
		$this->db->insert('aktiva_details', $data);
		$waktucek = substr($akhirbulan, 0, 7) .'-01';
		$bulanpertama = date("Y-m-t", strtotime($waktucek));
		//die('waktu cek = '.$waktucek);
		//insert tahun berikutnya
		for ($i=1;$i< $cari["konfigurasi"]["bagi"]*12;$i++) {
		    
		    $kode_unik = base_convert(microtime(false), 10, 36);
			$purchase_date_timestamp = strtotime($waktucek);
			//$purchase_date_1year = strtotime("+$i year", $purchase_date_timestamp);
			$purchase_date_1month = strtotime("+$i month", $purchase_date_timestamp);
			$jtempo = date("Y-m-d", $purchase_date_1month);
			
            $akhirbulan = date("Y-m-t", strtotime($jtempo));
